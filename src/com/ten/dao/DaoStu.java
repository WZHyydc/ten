...
public class DaoStu {
	...
	public List<Student> selectStu(){
		...
		try{
			String sql_select = "select s.sno,sname,password,classname,gender from student s join suser su on s.SNo = su.SNo join class c on s.classno = c.classno";//查询全部学生信息+学分学分;//查询所有的sql语句
			Connection conn = new Conn().getConn();
	...
	public Student selectStu(int id){
		Student Stu = null;
		try{
			String sql_selectone = "select * from student s left join suser su on s.SNO = su.SNO left join class c on c.classno = s.classno where s.SNO =?";//查询某一条学生信息
			Connection conn = new Conn().getConn();
	...
	public int updateStu(Student stu){
		int rs = 0;
		try{
			String sql_update = "update Suser set password=? where SNo=?";//修改一条学生信息
			Connection conn = new Conn().getConn();
	...
	public int deleteStu(int id){
		int rs = 0;
		try{
			String sql_delete = "delete from suser where SNo=?";//删除一条课程信息
			Connection conn = new Conn().getConn();

  ...
  <%
  		...
  		<%
				...
				if(ite.hasNext()&&type!=20){//查询结果不为空，输出表格
					...
					<table>
					<tr><td>学号</td><td>学生姓名</td><td>学生班级</td><td>教学班号</td><td>课程名</td><td>上课教室</td><td>修改教室</td></tr>
				...
				%> 
						...
						<td><%=tem.getSname() %></td>
						<td><%=tem.getSclass() %></td>
						<td><%=tem.getLessonNo() %></td>
						<td><%=tem.getCname() %></td>
						<td><%=tem.getClassroom() %></td>
						<td><input class="btn" type="button" value="修改教室" onclick="window.location.href=('/WebContent/servlet/serUpdateClass?sno=<%=tem.getSNo() %>&cno=<%=tem.getLessonNo() %>&type=<%= type %>&key=<%=queryKey%> ')"/></td></tr>

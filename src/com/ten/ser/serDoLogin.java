		...
		try{
			...
			if(type.equals("tea")){//判断是否为老师
				...
				if(tea!=null){
					...
					response.sendRedirect("/WebContent/tea/teacher.jsp");
				}else{
					out.println("T用户名或密码不正确，请核对后重试!!");
					response.setHeader("refresh","2;url=/WebContent/tea/teaLogin.jsp");
				}
				...
				if(stu!=null){
					...
					System.out.println("ljj"+basePath);
					session.setAttribute("student", stu);
					response.sendRedirect("/WebContent/stu/student.jsp");
					String path1 = request.getContextPath();
					String basePath1 = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
					System.out.println();
				}else{
					out.println("S用户名或密码不正确，请核对后重试!!");
					response.setHeader("refresh","2;url=/WebContent/Login.jsp");
				}
			}else{
				out.println("错误！请登录！");
				response.setHeader("refresh","2;url=/WebContent/ogin.jsp");
			}
		}catch(Exception e){ e.printStackTrace();response.sendRedirect("/WebContent/Login.jsp");}
		out.flush();
		out.close();
		}
